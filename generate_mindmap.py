    ...
    <style>
        ...
        * {{
        ...
        @media (max-width: 768px) {{
            ...
            /* World Map: maximise map space on mobile */
            #worldMapTab {{
                min-height: calc(100vh - 140px);
                height: calc(100vh - 140px);
            }}
            #worldMapTab .worldmap-controls-panel {{
                padding: 6px 10px;
                gap: 8px;
            }}
            #worldMapTab .worldmap-controls-panel h2 {{
                font-size: 1em;
            }}
            #worldMapTab .worldmap-main-area {{
                flex-direction: column;
                padding: 8px;
                gap: 8px;
                min-height: 0;
            ...
            #worldMapTab .worldmap-content {{
                flex: 1;
                min-height: 0;
                gap: 4px;
            }}
            #worldMapTab #worldMapContainer {{
                flex: 1;
                min-height: 50vh;
            }}
            #worldMapTab #worldMapContainer #worldMapWrapper {{
                min-height: 200px;
            }}
            #worldMapTab #worldMapSidebar {{
                width: 100%;
                min-width: 0;
                max-height: 35vh;
                flex: 0 0 auto;
        ...
        @media (max-width: 480px) {{
            ...
            /* World Map: even more space on small phones */
            #worldMapTab {{
...
"""
            ...
            try {{
                ...
                firebase.auth().signInAnonymously()
                    .then(function(userCredential) {{
                        ...
                        console.log("🔐 Auth token available for uploads");
                    ...
                    .catch(function(error) {{
                        ...
                        console.log("⚠️ Continuing without authentication - uploads will fail!");
                        setupRealtimeSync();
                    }});
        ...
        // DEPRECATED: Old function that causes data loss - use saveInventoryTransaction instead
        function saveToFirebase(path, data) {{
            console.warn("⚠️ saveToFirebase is deprecated - causes last-write-wins data loss");
            if (!database) {{
        ...
        function saveInventoryTransaction(localInventoryData) {{
            if (!database) {{
                // Fallback to localStorage
                try {{
                    localStorage.setItem('inventory_data', JSON.stringify(localInventoryData));
            ...
            ref.transaction(function(currentData) {{
                ...
                if (error) {{
                    ...
                    console.log("✅ Inventory saved with transaction (no data loss!)");
                    // Update local data with merged result
                    const mergedData = snapshot.val();
                    if (mergedData) {{
                        // Ensure merged data has valid structure before assigning
                        inventoryData = {{
                            partyGold: typeof mergedData.partyGold === 'number' ? mergedData.partyGold : 0,
                            players: Array.isArray(mergedData.players) ? mergedData.players : [],
                            bagOfHolding: Array.isArray(mergedData.bagOfHolding) ? mergedData.bagOfHolding : [],
                            version: mergedData.version || 0,
                        ...
                        if (typeof renderBagOfHolding === 'function') {{
                            renderBagOfHolding();
                        }}
                    }}
        ...
        // Merge player arrays intelligently
        function mergePlayers(remotePlayers, localPlayers) {{
            ...
            safeLocal.forEach(function(player) {{
                if (player && player.name) {{
                    if (playerMap[player.name]) {{
                        ...
                        playerMap[player.name].items = mergeItems(
                            playerMap[player.name].items || [],
                            player.items || []
                        );
            ...
            for (const name in playerMap) {{
                merged.push(playerMap[name]);
            }}
            
            return merged;
        }}
        
        // Merge item arrays intelligently (by _id)
        ...
        function saveMarkersTransaction(localMarkers, pathKey) {{
            ...
            ref.transaction(function(currentData) {{
                ...
                // Merge markers by id
                const safeRemote = Array.isArray(currentData) ? currentData : [];
        ...
        // Merge markers/annotations by id
        function mergeMarkers(remoteMarkers, localMarkers) {{
            const merged = [];
            const markerMap = {{}};
            
            // Ensure arrays exist
            const safeRemote = Array.isArray(remoteMarkers) ? remoteMarkers : [];
            const safeLocal = Array.isArray(localMarkers) ? localMarkers : [];
            
            // Add all remote markers
            safeRemote.forEach(function(marker) {{
                if (marker && marker.id) {{
            ...
            safeLocal.forEach(function(marker) {{
                if (marker && marker.id) {{
                    markerMap[marker.id] = marker;
                }}
            }});
        ...
        // NPC Firebase Functions
        let isSyncingNPCsFromFirebase = false;
        
        function loadNPCsFromFirebase(callback) {{
            if (!database) {{
                console.log("📂 No database - loading NPCs from static data");
                callback(null);
                return;
            }}
            ...
            try {{
                ...
                ref.once('value').then(function(snapshot) {{
                    ...
                    if (data && typeof data === 'object') {{
                        console.log("📂 Loaded NPCs from Firebase:", Object.keys(data).length, "NPCs");
                        callback(data);
                    }} else {{
                        console.log("📂 No NPC data in Firebase - using static data");
                        callback(null);
                    }}
                }}).catch(function(error) {{
                    console.error("❌ Error loading NPCs from Firebase:", error);
                    callback(null);
                }});
            }} catch(e) {{
                console.error("Failed to load NPCs from Firebase:", e);
                callback(null);
            }}
        }}
        
        function cleanNPCData(data) {{
            // Remove undefined values - Firebase doesn't accept them
            const cleaned = {{}};
            Object.keys(data).forEach(function(npcId) {{
                ...
                Object.keys(npc).forEach(function(key) {{
                    ...
                    if (value === '' && (key === 'faction' || key === 'location' || key === 'portrait' || key === 'notes')) return;
                    // Skip empty arrays
                    if (Array.isArray(value) && value.length === 0) return;
                    // Skip empty objects
                    if (typeof value === 'object' && !Array.isArray(value) && Object.keys(value).length === 0) return;
                    
                    cleanedNPC[key] = value;
                }});
                
                // Ensure name exists
                if (!cleanedNPC.name) {{
                    cleanedNPC.name = npcId;
                }}
                
                cleaned[npcId] = cleanedNPC;
            }});
        ...
        function saveNPCsToFirebase(npcData) {{
            ...
            if (isSyncingNPCsFromFirebase) {{
                console.log("🔄 Skipping NPC save - currently syncing from Firebase");
                return;
            }}
        ...
        function migrateNPCsToFirebase() {{
            if (!database) {{
                console.log("No database - skipping NPC migration");
                return;
            }}
            
            // Check if NPCs already exist in Firebase
            loadNPCsFromFirebase(function(firebaseData) {{
                if (firebaseData && Object.keys(firebaseData).length > 0) {{
                    console.log("✅ NPCs already in Firebase - no migration needed");
                    return;
                }}
                
                // Migrate from static npcData to Firebase
                if (typeof npcData !== 'undefined' && npcData && Object.keys(npcData).length > 0) {{
                    console.log("🔄 Migrating", Object.keys(npcData).length, "NPCs to Firebase...");
                    saveNPCsToFirebase(npcData);
                    console.log("✅ NPC migration complete");
                }} else {{
                    console.log("⚠️ No static NPC data to migrate");
                }}
            }});
        }}
        
        // Dynamic NPC Card Rendering
        function renderNPCCards() {{
            const npcGrid = document.getElementById('npcGrid');
            if (!npcGrid) {{
                console.log("NPC grid not found - skipping render");
                return;
            }}
            
            console.log("🎴 Rendering NPC cards from editorNPCData:", Object.keys(editorNPCData).length, "NPCs");
            
            // Clear existing cards
            npcGrid.innerHTML = '';
            
            // Sort NPCs alphabetically by name
            const sortedNPCs = Object.keys(editorNPCData).sort(function(a, b) {{
                const nameA = (editorNPCData[a].name || a).toLowerCase();
                const nameB = (editorNPCData[b].name || b).toLowerCase();
            ...
            sortedNPCs.forEach(function(npcId) {{
                ...
                const spoiler = npc.spoiler || false;
                const relationships = npc.relationships || {{}};
                
                // Get faction color
                const factionColors = {
                    'The Veil': '#9C27B0',
                    'Duskkeep Guild': '#3F51B5',
                    'The Forgotten Assembly': '#F44336',
                    'Thenmere Council': '#2196F3',
                    'Thenmere Guard': '#4CAF50',
                    'Independent': '#FF9800',
                    'Party Members': '#FFD700',
                    'Unknown': '#808080'
                };
                const factionColor = factionColors[faction] || '#808080';
                ...
                if (tags && tags.length > 0) {{
                    const tagsList = tags.map(function(t) {{ return '<span class="tag">' + t + '</span>'; }}).join('');
                    tagsHtml = '<div class="npc-tags"><span class="tag-label">Tags:</span> ' + tagsList + '</div>';
                }}
                
                // Build relationships HTML
                let relationshipsHtml = '';
                const relCount = Object.keys(relationships).reduce(function(sum, key) {{
                    const rels = relationships[key];
                    return sum + (Array.isArray(rels) ? rels.length : 0);
                }}, 0);
                
                if (relCount > 0) {{
                    relationshipsHtml = '<div class="npc-relationships"><strong>Relationships:</strong> <span class="relationship-count">' + relCount + '</span></div>';
                }}
                
                // Create card element
                const card = document.createElement('div');
                card.className = 'npc-card';
                card.setAttribute('data-faction', faction);
                card.setAttribute('data-status', status);
                card.setAttribute('data-name', name.toLowerCase());
                card.setAttribute('data-location', location.toLowerCase());
                card.setAttribute('data-tags', tags.join(',').toLowerCase());
                card.setAttribute('data-spoiler', spoiler.toString());
                card.setAttribute('data-portrait', portrait);
                
                const notesHtml = notes ? '<div class="npc-notes"><strong>📝 Notes:</strong> ' + notes + '</div>' : '';
                
                card.innerHTML = '<div class="npc-header">' +
                    '<div class="npc-portrait-container">' +
                    '<div class="npc-portrait" style="' + portraitStyle + '">' +
                    portraitHtml +
                    '</div>' +
                    '</div>' +
                    '<div class="npc-name-container">' +
                    '<h3 class="npc-name">' + name + '</h3>' +
                    '<span class="npc-faction" style="background-color: ' + factionColor + ';">' + faction + '</span>' +
                    '</div>' +
                    '</div>' +
                    '<div class="npc-details">' +
                    '<p class="npc-location"><strong>📍 Location:</strong> ' + location + '</p>' +
                    '<p class="npc-status"><strong>💚 Status:</strong> ' + status + '</p>' +
                    tagsHtml +
                    relationshipsHtml +
                    notesHtml +
                    '</div>';
            ...
            console.log("✅ NPC cards rendered successfully");
        }}
            ...
            npcRef.on('value', function(snapshot) {{
                ...
                if (data && typeof data === 'object') {{
                    ...
                    if (currentDataStr !== newDataStr) {{
                        ...
                        // Update editorNPCData
                        editorNPCData = data;
                        
                        // Update NPC list in editor if it's open
                        if (typeof populateNPCList === 'function') {{
                            populateNPCList();
                        }}
                        
                        // Re-render NPC cards in main view
                        if (typeof renderNPCCards === 'function') {{
                            renderNPCCards();
                        }}
                ...
                console.error("❌ Error listening to NPC data:", error);
            }});
        ...
        (function() {{
                    ...
                    try {{
                        ...
                        if (isHtmlFormat && mapCanvas) {{
                            ...
                            if (canvasEl) {{
                                ...
                                // Try to convert to high-resolution canvas for better quality
                                if (isDebugMode()) {{
                                    console.log('   - HTML format detected, attempting high-res canvas conversion');
                                }}
                                ...
                                // Try to find and extract the main image
                                const tempDiv = document.createElement('div');
            ...
            function updateMapTransform() {{
                ...
                requestAnimationFrame(function() {{
                    ...
                    if (isCanvasMode && mapCanvas) {{
                        // Canvas is already centered with CSS (50% + translate(-50%, -50%))
            ...
            mapContainer.addEventListener('wheel', function(e) {{
                ...
                // Get viewport center (mapContainer center)
            ...
            mapContainer.addEventListener('touchmove', function(e) {{
                ...
                if (touches.length === 1 && isDragging && !isPinching) {{
                    ...
                    if (distanceChange > 5) {{
                        ...
                        // Update last touch center for two-finger pan
                            ...
                            <div class="form-group">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                    ...
                                    <div style="font-size: 11px; color: rgba(255,255,255,0.5); margin-top: -5px;">
                                        💡 Click Upload to add images directly, or enter URL/path manually
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        // Load NPCs from Firebase, fallback to static data
        function initializeNPCData() {{
            loadNPCsFromFirebase(function(firebaseData) {{
                if (firebaseData && Object.keys(firebaseData).length > 0) {{
                    // Use Firebase data
                    editorNPCData = firebaseData;
                    console.log('✅ Editor initialized with Firebase NPCs:', Object.keys(editorNPCData).length, 'NPCs');
                }} else if (typeof npcData !== 'undefined' && npcData && Object.keys(npcData).length > 0) {{
                    // Fallback to static data and migrate to Firebase
                    editorNPCData = JSON.parse(JSON.stringify(npcData)); // Working copy
                    console.log('✅ Editor initialized with static NPCs:', Object.keys(editorNPCData).length, 'NPCs');
                    console.log('🔄 Migrating NPCs to Firebase...');
                    migrateNPCsToFirebase();
                }} else {{
                    console.error('❌ No NPC data available');
                    editorNPCData = {{}};
                }}
                
                // Populate NPC list in editor if tab is open
                if (typeof populateNPCList === 'function') {{
                    populateNPCList();
                }}
                
                // Render NPC cards in main view
                if (typeof renderNPCCards === 'function') {{
                    renderNPCCards();
                }}
            }});
        ...
        // Initialize NPC data on load
        if (typeof database !== 'undefined' && database) {{
            // Wait for Firebase to be ready
            setTimeout(initializeNPCData, 500);
        }} else {{
            // No Firebase - use static data immediately
            if (typeof npcData !== 'undefined' && npcData && Object.keys(npcData).length > 0) {{
                editorNPCData = JSON.parse(JSON.stringify(npcData)); // Working copy
                console.log('Editor initialized with', Object.keys(editorNPCData).length, 'NPCs (no Firebase)');
            }} else {{
        ...
        function saveNPC(event) {
            ...
            // Create or update NPC (only include defined values - Firebase doesn't accept undefined)
            const npcObj = { name: name };
            
            if (faction && faction.trim()) npcObj.faction = faction;
            if (location && location.trim()) npcObj.location = location;
            npcObj.status = status || 'alive';
            if (portrait && portrait.trim()) npcObj.portrait = portrait;
            if (tags && tags.length > 0) npcObj.tags = tags;
            if (notes && notes.trim()) npcObj.notes = notes;
            if (spoiler) npcObj.spoiler = true; // Only include if true
            if (Object.keys(relationships).length > 0) npcObj.relationships = relationships;
            
            editorNPCData[npcId] = npcObj;
            ...
            if (database) {
                showNotification('✅ NPC saved and synced to all players!');
            } else {
                alert('NPC saved locally (Firebase not available)');
            }
        }
        ...
        function deleteNPC() {
            ...
            if (database) {
                showNotification('🗑️ NPC deleted and synced to all players');
            }
            
            // Show success message with Firebase sync info
            if (database) {
                showNotification('✅ NPC deleted and synced to all players!');
            } else {
                alert('NPC deleted locally (Firebase not available)');
            }
        }
                ...
                uploadTask.on('state_changed',
                    ...
                    function() {
                        ...
                        uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                            ...
                            // Update the NPC data with new portrait URL
                            if (currentEditingNPC && editorNPCData[currentEditingNPC]) {
                                editorNPCData[currentEditingNPC].portrait = downloadURL;
                                ...
                                console.log('✅ Portrait updated in card view');
                            }
                            
                            showUploadStatus('✅ Image uploaded and card updated!', 'success');
                        }).catch(function(error) {
        ...
        function debouncedSaveInventoryData() {{
            ...
            saveInventoryTimeout = setTimeout(function() {{
                try {{
                    ...
                    // Use transaction-based save to prevent data loss
                    saveInventoryTransaction(inventoryData);
                    showNotification('✅ Saved (transaction-safe)', 'success');
                    savePending = false;
                }} catch(e) {{
