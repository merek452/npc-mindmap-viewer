...
"""
    ...
    <script>
        ...
        function renderItem(item, containerId, index) {{
            ...
            return `
                <div class="inventory-item" 
                     ...
                     ontouchstart="if(event.touches.length === 1) handleTouchStart(event, JSON.parse('${{itemForTouch}}'), '${{containerId}}')"
                     ontouchmove="if(event.touches.length === 1) handleTouchMove(event)"
                     ontouchend="if(event.changedTouches.length === 1) handleTouchEnd(event)"
                     data-item-id="${{itemId}}"
        ...
        // Mobile: Handle drop - simplified
        function handleTouchEnd(ev) {
            try {
                const element = ev.currentTarget || ev.target;
                if (!element || !element._touchItem) {
                    // Reset any element that might have touch data
                    if (element) {
                        element.style.opacity = '';
                        delete element._touchItem;
                    }
                    return;
                }
                ...
                if (element._touchMoved || deltaX > 3 || deltaY > 3) {
                    ev.preventDefault();
                    ev.stopPropagation();
                    ...
                    try {
                        showItemDescription(element._touchItemData);
                    } catch(e) {
                ...
                // Clean up
                delete element._touchItem;
                delete element._touchSource;
                delete element._touchItemData;
                delete element._touchStartX;
                delete element._touchStartY;
                delete element._touchMoved;
            } catch(e) {
