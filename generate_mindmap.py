...
"""
    ...
    <script>
            ...
            if (touchMoved || deltaX > 20 || deltaY > 20) {
                ...
                if (!finalTargetContainer) {
                    ...
                    if (targetElement) {
                        ...
                        if (dropContainer) {
                            // Get the container ID - check multiple ways
                            if (dropContainer.dataset && dropContainer.dataset.containerId) {
                                finalTargetContainer = dropContainer.dataset.containerId;
                            } else if (dropContainer.id) {
                                // If ID is like "player_0_container", extract "player_0"
                                if (dropContainer.id.endsWith('_container') && dropContainer.id.startsWith('player_')) {
                                    finalTargetContainer = dropContainer.id.replace('_container', '');
                                ...
                                // If no ID, look for parent with player_ ID or data attribute
                                const parentWithId = dropContainer.closest('[id^="player_"], [data-container-id]');
                                if (parentWithId) {
                                    if (parentWithId.dataset && parentWithId.dataset.containerId) {
                                        finalTargetContainer = parentWithId.dataset.containerId;
                                    } else if (parentWithId.id) {
                                        if (parentWithId.id.endsWith('_container')) {
                                            finalTargetContainer = parentWithId.id.replace('_container', '');
                                        } else {
                ...
                if (finalTargetContainer) {
                    logger.log('Mobile drag: Dropping item into container:', finalTargetContainer);
                    logger.log('Item data:', touchStartItem ? touchStartItem.name : 'null');
                    ...
                    const fakeEvent = {{
                        ...
                        dataTransfer: {{
                            getData: function(key) {{
                                if (key === 'item') {{
                                    logger.log('getData(item):', touchStartItemData ? 'has data (' + touchStartItemData.length + ' chars)' : 'no data');
                                    return touchStartItemData;
                                }}
                                if (key === 'source') {{
                                    const src = touchStartContainer || 'lookup';
                                    logger.log('getData(source):', src);
                                    return src;
                                }}
                    ...
                    try {{
                        ...
                        logger.log('Drop function called successfully');
                        
                        // Force UI update after a short delay to ensure data is saved
                        setTimeout(function() {{
                            if (finalTargetContainer === 'bagOfHolding') {{
                                renderBagOfHolding();
                            }} else if (finalTargetContainer.startsWith('player_')) {{
                                renderPlayers();
                            }}
                        }}, 100);
                    }} catch(e) {{
                        logger.error('Error in drop function:', e);
                        logger.error('Stack:', e.stack);
                    }}
                }} else {{
                    logger.warn('Mobile drag: Could not find drop target at', touch.clientX, touch.clientY);
                    logger.warn('Target element:', targetElement ? targetElement.tagName + (targetElement.className ? '.' + targetElement.className : '') + (targetElement.id ? '#' + targetElement.id : '') : 'null');
                }
            }
        ...
        function drop(ev, targetContainer) {
            ...
            // Check if dragging from lookup table
