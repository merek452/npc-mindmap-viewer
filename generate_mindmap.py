...
"""
    ...
    <script>
        ...
        function renderItem(item, containerId, index) {{
            ...
            // Escape for HTML attribute (double quotes)
            const itemJsonForAttr = JSON.stringify(item).replace(/"/g, '&quot;').replace(/'/g, '&#39;');
            
            return `
                <div class="inventory-item" 
                     draggable="true" 
                     ondragstart="drag(event, '${{itemId}}', '${{containerIdEscaped}}')"
                     onclick="if(!event.target.closest('button')) { const itemData = this.dataset.itemJson ? JSON.parse(this.dataset.itemJson.replace(/&quot;/g, '\\"').replace(/&#39;/g, \"'\")) : null; if(itemData) showItemDescription(itemData); }"
                     ...
                     data-item-json="${{itemJsonForAttr}}"
